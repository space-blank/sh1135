...
function get_ueditor($editor_name,$BasePath='../include/ueditor')
{
	$html = '';  
	$html .= '<script src="'.$BasePath.'/ueditor.config.js"></script><script  src="'.$BasePath.'/ueditor.all.js"></script>'; 
	$html .='<textarea name="'.$editor_name.'" id="myEditor" require="true" datatype="limit" msg="请填写信息内容描述" msgid="content">'.$value.'</textarea> ';  
...
//	$data = read_static_cache('seoset');
	$data = false;
	if($data === false){
		$query = $GLOBALS['db']->select("SELECT description,value FROM `{$GLOBALS['db_mymps']}config` WHERE type = 'seo'");
...
function get_upload_image_view($if_upimg = 1 , $infoid = '')
{
	...
	<input class="input" id="insetIMG" name="imgs[]" type="file" multiple="multiple"  datatype="filter" msg="图片文件格式不正确">
	<input class="input" type="file"  name='mymps_img_0' datatype="filter" style="display:none" msg="图片文件格式不正确">
	<br>
<script type="text/javascript">
	$("#insetIMG").change(function(){
		$('.tempss').remove();
		var imageSL='<div class="tempss" id="mytest" style="padding: 10px;display: -webkit-inline-box;padding-left: 0px;height:80px">';
		for(var i=0;i<this.files.length;i++){
			imageSL+='<img class="img1" src="'+getObjectURL(this.files[i])+'" width=80 height=80 style="margin-left:10px">';
		}
		if (imageSL!='') {
...
$("#insetIMG").bind('change',function(){
		
		var oUl= document.getElementById("mytest");
			...
			obj.onmousedown = function(event){
				...
				document.onmouseup = function(){
					...
					if(oNear){
						...
						//alert($("#insetIMG")[0].files);
						
						for(var i in $("#insetIMG")[0].files){
						console.log($("#insetIMG")[0].files[i]);
						}
						var p=$("#insetIMG")[0].files[0];
						$("#insetIMG")[0].files[0]=$("#insetIMG")[0].files[1];
						$("#insetIMG")[0].files[1]=p;
						...
						var items_array = $('.img1').val();
						var items = items_array.split(',');
						//alert(items_array);
						for (var i in items){
							var item_number = items[i];
							//alert(storedFiles[item_number]);
...
function mymps_get_infos($num=10,$info_level=NULL,$upgrade_type=NULL,$userid=NULL,$catid=NULL,$certify=NULL,$if_hot=NULL,$tel=NULL,$cityid=NULL){
	...
	if($idin){
		$sql = "SELECT a.id,a.contact_who,a.danwei,a.title,a.content,a.begintime,a.catid,a.info_level,a.hit,a.dir_typename,a.ifred,a.ifbold,a.userid,a.catid,a.cityid,a.catname,a.img_path FROM `{$GLOBALS['db_mymps']}information` AS a WHERE id IN ($idin) {$orderby}";
		$do_mymps = $db -> query($sql);
		while($row = $db -> fetchRow($do_mymps)){
			$arr['id']        = $row['id'];
...
function mymps_get_info($num=10,$info_level=NULL,$upgrade_type=NULL,$userid=NULL,$catid=NULL,$certify=NULL,$if_hot=NULL,$cityid=NULL){
	global $timestamp,$db_mymps,$mymps_global,$db,$city,$seo;
	$where .= !$info_level ? 'WHERE (a.info_level =1 OR a.info_level = 2)':'WHERE a.info_level = '.$info_level;
	$where .= $userid	? ' AND a.userid = "'.$userid.'" ' : '';
	$where .= $certify	? ' AND a.certify = "'.$certify.'" ' : '';
	...
	if($idin){
		$sql = "SELECT a.id,a.contact_who,a.title,a.content,a.begintime,a.catid,a.info_level,a.hit,a.dir_typename,a.ifred,a.ifbold,a.userid,a.tel,a.catid,a.cityid,a.catname,a.img_path FROM `{$GLOBALS['db_mymps']}information` AS a WHERE id IN ($idin) {$orderby}";
		$do_mymps = $db -> query($sql);
		while($row = $db -> fetchRow($do_mymps)){
			$arr['id']        = $row['id'];
			$arr['catid']     = $row['catid'];
...
function mymps_get_hot($num=10,$info_level=NULL,$upgrade_type=NULL,$userid=NULL,$catid=NULL,$certify=NULL,$cityid=NULL){
	global $timestamp,$db_mymps,$mymps_global,$db,$city,$seo;
	$where .= !$info_level ? 'WHERE (a.info_level =1 OR a.info_level = 2)':'WHERE a.info_level = '.$info_level;
	$where .= $userid	? ' AND a.userid = "'.$userid.'" ' : '';
	$where .= $certify	? ' AND a.certify = "'.$certify.'" ' : '';
...
function hanwa888($num=10,$cityid=NULL){
	global $timestamp,$db_mymps,$mymps_global,$db,$city,$seo;
	$info_list = array();
	//$sql = "SELECT a.id,a.contact_who,a.danwei,a.areaid,a.title,a.content,a.begintime,a.catid,a.info_level,a.hit,a.dir_typename,a.ifred,a.ifbold,a.userid,a.tel,a.catid,a.cityid,a.catname,a.img_path FROM `{$GLOBALS['db_mymps']}information` AS a LEFT JOIN `{$GLOBALS['db_mymps']}category` AS b ON a.catid = b.catid WHERE b.parentid IN(528,529,530,532,534,535,539) AND a.info_level = 2 AND a.cityid = $cityid ORDER BY a.hit DESC,$num";
	$sql = "SELECT a.id,a.contact_who,a.danwei,a.areaid,a.title,a.content,a.begintime,a.catid,a.info_level,a.hit,a.dir_typename,a.ifred,a.ifbold,a.userid,a.tel,a.catid,a.cityid,a.catname,a.img_path FROM `{$GLOBALS['db_mymps']}information` AS a LEFT JOIN `{$GLOBALS['db_mymps']}category` AS b ON a.catid = b.catid WHERE a.info_level = 2 AND a.cityid = $cityid ORDER BY a.hit DESC limit $num";
	$do_mymps = $db -> query($sql);
	while($row = $db -> fetchRow($do_mymps)){
		$arr['desc'] = $db->getRow("select * from ".$GLOBALS['db_mymps']."information_25 where id=".$row['id']);
		if($arr['desc']==""){
			$arr['desc'] = $db->getRow("select * from ".$GLOBALS['db_mymps']."information_28 where id=".$row['id']);
		}
		if($arr['desc']==""){
			$arr['desc'] = $db->getRow("select * from ".$GLOBALS['db_mymps']."information_35 where id=".$row['id']);
		}
		if($arr['desc']==""){
			$arr['desc'] = $db->getRow("select * from ".$GLOBALS['db_mymps']."information_29 where id=".$row['id']);
		}
		//if($row['id']==58092){
			//die(print_r("select * from ".$GLOBALS['db_mymps']."information_25 where id=".$row['id']));
		//}
		$street = $db->getOne("select streetname from ".$GLOBALS['db_mymps']."street where areaid=".$row['areaid']);
		$arr['areaname']	 = get_areaname($row['areaid']).$street;
		//echo "select * from ".$GLOBALS['db_mymps']."information_25 where id=".$arr['id'];
		$arr['id']        = $row['id'];
		$arr['danwei']    = $row['danwei'];
		$arr['catid']     = $row['catid'];
		$arr['title']     = $row['title'];
...
function hanwa($num=13,$cityid=NULL){
	global $timestamp,$db_mymps,$mymps_global,$db,$city,$seo;
	$info_list = array();
	$sql = "SELECT a.id,a.contact_who,a.title,a.danwei,a.areaid,a.content,a.begintime,a.catid,a.info_level,a.hit,a.dir_typename,a.ifred,a.ifbold,a.userid,a.tel,a.catid,a.cityid,a.catname,a.img_path FROM `{$GLOBALS['db_mymps']}information` AS a LEFT JOIN `{$GLOBALS['db_mymps']}category` AS b ON a.catid = b.catid WHERE a.info_level = 2 AND a.cityid = $cityid ORDER BY a.hit DESC,$num";
	$do_mymps = $db -> query($sql);
	while($row = $db -> fetchRow($do_mymps)){
		$arr['desc'] = $db->getRow("select * from ".$GLOBALS['db_mymps']."information_25 where id=".$row['id']);
		$street = $db->getOne("select streetname from ".$GLOBALS['db_mymps']."street where areaid=".$row['areaid']);
...
function mymps_get_scroll($num=10,$upgrade_type_index=NULL,$cityid=NULL){
	global $timestamp,$db_mymps,$mymps_global,$db,$city,$seo;
	$where .= 'WHERE a.upgrade_type_index = '.$upgrade_type_index;
	$where .= $cityid > 0 ? ' AND a.cityid = "'.$cityid.'" ' : '';
	$orderby =  " ORDER BY a.hit DESC " ;
	$info_list = array();
	$idin = get_page_idin("id","SELECT a.id FROM `{$db_mymps}information` AS a {$where} {$orderby}",$num);
...
function mymps_get_upindex($num=10,$upindex=NULL,$slen=20,$upgrade_type=NULL,$userid=NULL,$catid=NULL,$certify=NULL,$cityid=NULL){

	global $timestamp,$db_mymps,$mymps_global,$db,$city,$seo;
	$where .= 'WHERE a.upindex = '.$upindex;
	$where .= $userid	? ' AND a.userid = "'.$userid.'" ' : '';
	$where .= $certify	? ' AND a.certify = "'.$certify.'" ' : '';
	$where .= $catid	? ' AND '.get_children($catid,'a.catid') : '';
	$where .= $cityid > 0 ? ' AND a.cityid = "'.$cityid.'" ' : '';
	$cat = $db->getrow( "SELECT modid FROM `".$db_mymps."category`  WHERE catid = '{$catid}'" );
	//var_dump($cat);
	if($upgrade_type == 1){
		$where .= " AND a.upgrade_type_list = 2 ";
	} elseif($upgrade_type == 2){
	...
	if($idin){
	$sql = "SELECT a.id,a.contact_who,a.danwei,a.areaid,a.title,a.content,a.begintime,a.catid,a.info_level,a.hit,a.dir_typename,a.ifred,a.ifbold,a.userid,a.tel,a.catid,a.cityid,a.catname,a.img_path,b.areaname,c.* FROM `{$GLOBALS['db_mymps']}information` AS a 
	LEFT JOIN `{$db_mymps}area` AS b ON a.areaid = b.areaid  
	LEFT JOIN `{$db_mymps}information_{$cat[modid]}` AS c ON a.id = c.id  WHERE a.id IN ($idin) {$orderby}";
		...
		while($row = $db -> fetchRow($do_mymps)){
			...
			$arr['catid']     = $row['catid'];
			$arr['title']     = $row['title'];
			$arr['stitle']     = mb_substr($row['title'],0,$slen,"gbk");
			$arr['content']   = clear_html($row['content']);
			$arr['ifred']     = $row['ifred'];
			$arr['ifbold']    = $row['ifbold'];
			$arr['hit']    	  = $row['hit'];
			...
			$arr['acreage'] = $row['acreage'];
			if($row['rent']){
				$arr['rent'] = $row['rent'];
			}elseif($row['store_zs']){
				$arr['rent'] = $row['store_zs'];
			}else{
				$arr['rent'] = 0;
			}
			//$arr['rent'] = $row['rent'];
...
function mymps_get_upindex_cang($num=10,$upindex=NULL,$slen=20,$upgrade_type=NULL,$userid=NULL,$catid=NULL,$certify=NULL,$cityid=NULL){
	global $timestamp,$db_mymps,$mymps_global,$db,$city,$seo;
	$where .= 'WHERE a.upindex = '.$upindex;
	$where .= $userid	? ' AND a.userid = "'.$userid.'" ' : '';
	$where .= $certify	? ' AND a.certify = "'.$certify.'" ' : '';
	...
	if($idin){
	$sql = "SELECT a.id,a.contact_who,a.title,a.content,a.begintime,a.catid,a.info_level,a.hit,a.dir_typename,a.ifred,a.ifbold,a.userid,a.tel,a.catid,a.cityid,a.catname,a.img_path,b.areaname,c.acreage,c.store_zs FROM `{$GLOBALS['db_mymps']}information` AS a 
	LEFT JOIN `{$db_mymps}area` AS b ON a.areaid = b.areaid  
	LEFT JOIN `{$db_mymps}information_35` AS c ON a.id = c.id  WHERE a.id IN ($idin) {$orderby}";
		$do_mymps = $db -> query($sql);
		while($row = $db -> fetchRow($do_mymps)){
			$arr['id']        = $row['id'];
			$arr['catid']     = $row['catid'];
...
function mymps_get_upindex_fang($num=10,$upindex=NULL,$slen=20,$upgrade_type=NULL,$userid=NULL,$catid=NULL,$certify=NULL,$cityid=NULL){
	global $timestamp,$db_mymps,$mymps_global,$db,$city,$seo;
	$where .= 'WHERE a.upindex = '.$upindex;
	$where .= $userid	? ' AND a.userid = "'.$userid.'" ' : '';
	$where .= $certify	? ' AND a.certify = "'.$certify.'" ' : '';
	...
	if($idin){
	$sql = "SELECT a.id,a.contact_who,a.title,a.content,a.begintime,a.catid,a.info_level,a.hit,a.dir_typename,a.ifred,a.ifbold,a.userid,a.tel,a.catid,a.cityid,a.catname,a.img_path,b.areaname,c.acreage,c.all_price FROM `{$GLOBALS['db_mymps']}information` AS a 
	LEFT JOIN `{$db_mymps}area` AS b ON a.areaid = b.areaid  
	LEFT JOIN `{$db_mymps}information_23` AS c ON a.id = c.id  WHERE a.id IN ($idin) {$orderby}";
		$do_mymps = $db -> query($sql);
		while($row = $db -> fetchRow($do_mymps)){
			$arr['id']        = $row['id'];
			$arr['catid']     = $row['catid'];
...
function getfoottext($cityid=0,$title=''){
	$where .= $cityid ? " AND cityid = '".$cityid."'" : "";
	$sql="select * from `{$GLOBALS['db_mymps']}advertisement` where title='".$title."' ".$where;
	$res = $GLOBALS['db'] -> getRow($sql);
	$res['parameters']=unserialize($res['parameters']);
	//die(print_r($res['parameters']));
	return $res;
}
...
function getheadgg(){
	$sql="select * from {$GLOBALS['db_mymps']}advertisement where title='头部广告小图'";
	$adv=$GLOBALS['db']->getRow($sql);
	return $adv;
}

function getheadhfgg(){
	$sql="select * from {$GLOBALS['db_mymps']}advertisement where title='头部横幅广告'";
...
function get_children_cats($cats){
	global $db,$db_mymps,$mymps_global;
	$query = $db -> query("SELECT * FROM `{$db_mymps}category` WHERE catid ".create_in($cats)." AND if_view = '2' ORDER BY catorder ASC");
	while($row = $db -> fetchRow($query)){
		$listcats[$row['catid']]['catid'] = $row['catid'];
